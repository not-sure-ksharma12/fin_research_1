...
class NVDAHestonStrategy:
    ...
    def load_nvda_data(self) -> pd.DataFrame:
        """
        Load NVDA options data from existing files
        
        Returns:
            DataFrame with options data including Heston prices
        """
        try:
            # Try to load from the most recent NVDA data file
            data_file = "scripts/excels/nvda_options_2025-08-15_heston.xlsx"
            ...
            if not os.path.exists(data_file):
                ...
                if not os.path.exists(data_file):
                    logger.error("No NVDA data files found")
                    return pd.DataFrame()
            
            logger.info(f"Loading NVDA data from {data_file}")
            df = pd.read_excel(data_file)
            
            # Ensure required columns exist
            required_columns = ['Strike', 'Option Type', 'PX_LAST', 'Heston_Price']
            ...
            df['Market_vs_Heston'] = df['PX_LAST'] - df['Heston_Price']
            df['Heston_vs_Market'] = df['Heston_Price'] - df['PX_LAST']
            
            # Add current price if not present
            if 'Current Price' not in df.columns:
                df['Current Price'] = df['PX_LAST'].mean() * 10  # Rough estimate
            
            logger.info(f"Loaded {len(df)} call options for {self.company}")
            return df
            
        except Exception as e:
            logger.error(f"Error loading NVDA data: {e}")
            return pd.DataFrame()
    ...
    def run_simulation(self, num_hours: int = 24) -> Dict:
        """
...
def main():
    ...
    # Run simulation
    results = strategy.run_simulation(num_hours=24)
    
    if results:
        logger.info("NVDA simulation completed successfully!")
        ...
        logger.error("NVDA simulation failed!")
