...
def fix_excel_file_simple(excel_filename, active_trades, exited_trades):
    """Fix Excel file with both active and exited trades - simple version"""
        ...
        for sheet_name in wb.sheetnames:
            if sheet_name.startswith('Hour_'):
                ...
                if option_id_col and trade_status_col and trade_type_col and position_size_col:
                    # Clear all trade status values first
                    ...
                    # Now mark the correct trades for this hour
                    for row in range(2, ws.max_row + 1):
...
def main():
    ...
    logger.info("=" * 80)
    logger.info("🔧 FIXING MAIN STRATEGY EXCEL FILES (SIMPLE)")
    logger.info("=" * 80)
    ...
    for company in companies:
        ...
        fix_excel_file_simple(excel_file, active_trades, exited_trades)
    
    logger.info("=" * 80)
    logger.info("✅ EXCEL FILES FIXED SIMPLY")
    logger.info("=" * 80)
